...
if ok
    if isfield(EEG.chaninfo,'ndchanlocs') && isstruct(EEG.chaninfo.ndchanlocs)
        ...
        refchanidx = find(strcmp(EEG.chaninfo.refchan,{EEG.chaninfo.ndchanlocs.labels}));
    ...
    switch refmode
        case 1
            ...
            if isempty(refchanidx)
                ...
                for ind = 1:length(fieldloc)
                    if ~isfield(EEG.chaninfo.ndchanlocs(refchanidx),fieldloc{ind})
                        EEG.chaninfo.ndchanlocs(refchanidx).(fieldloc{ind}) = [];
                    end
                end
                EEG = pop_reref( EEG, [], 'exclude', badchannels,'refloc',EEG.chaninfo.ndchanlocs(refchanidx));
                EEG.chaninfo.ndchanlocs(strcmp(EEG.chaninfo.refchan,{EEG.chaninfo.ndchanlocs.labels})) = [];
            end
            EEG.ref = 'averef';
        ...
        case 2
            ...
            if ~isempty(refchanidx)
                EEG.chanlocs(end+1).labels = EEG.chaninfo.ndchanlocs(refchanidx).labels;
                fieldloc = fieldnames(EEG.chaninfo.ndchanlocs(refchanidx));
                for ind = 1:length(fieldloc)
                    EEG.chanlocs(end).(fieldloc{ind}) = EEG.chaninfo.ndchanlocs(refchanidx).(fieldloc{ind});
                end
                EEG.chanlocs(end).type = '';
                EEG.chaninfo.ndchanlocs(refchanidx) = [];
                EEG.data(end+1,:,:) = 0;
        ...
        case 3
            ...
            if isempty(refchanidx)
                ...
                EEG = pop_reref( EEG, refchan, 'exclude', badchannels,'refloc',EEG.chaninfo.ndchanlocs(refchanidx));
                EEG.chaninfo.ndchanlocs(strcmp(EEG.chaninfo.refchan,{EEG.chaninfo.ndchanlocs.labels})) = [];
            end
        ...
        case 5
            ...
            if ~isempty(refchanidx)
                ...
                EEG.chanlocs(end).type = '';
                EEG.chaninfo.ndchanlocs(refchanidx) = [];
                EEG.chaninfo.nodatchans(refchanidx) = [];
                EEG.data(end+1,:,:) = 0;
                EEG.nbchan = EEG.nbchan + 1;
            end
            % EEGLAB has nose direction as X-axis and right ear direction
